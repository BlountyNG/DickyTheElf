...
{	
	...
	[Header("Default volume levels, by type (Music, SFX, Dialog)")]
	[ReorderableArray]
	...
	static public AudioHandle Play(AudioCue cue, ref List<AudioHandle> handles, Transform emmitter = null, float volumeMult = 1, float pitchMult = 1, float fromTime = 0 )
	{
		...
		// Apply type volumes- Currently simplified to just Music or Sound
		for ( int i = 0; i < self.m_volumeByType.Count; ++i )
		{
	...
	static public AudioHandle Play( AudioClip clip, int type = (int)AudioCue.eAudioType.Sound, Transform emmitter = null, float volume = 1, float pitch = 1, bool loop = false )
	{
		if ( clip == null)
			return null;
	...
	public static AudioHandle PlayMusic( string name, float fadeTime = 0 ) { return PlayMusic(name, fadeTime, fadeTime); }
	/// Play a music track using the cue name, with seperate fade out and fade in times
	public static AudioHandle PlayMusic( string name, float fadeOutTime, float fadeInTime ) 
	{
		return PlayMusic( m_instance.m_audioCues.Find(item=>string.Equals(name, item.name, System.StringComparison.OrdinalIgnoreCase)), fadeOutTime, fadeInTime );
	}
