...
public class PropComponent : MonoBehaviour 
{
	...
	[Tooltip("<0 = closer, 0 = none, 0->1 = further")]
...
public partial class Prop : IQuestClickable, IProp, IQuestScriptable
{
	...
	IEnumerator CoroutinePlayAnimation(string animName) 
	{
		...
		while ( m_instance.GetAnimating() && PowerQuest.Get.GetSkipCutscene() == false )
		{				
	...
	IEnumerator CoroutinePlayVideo(float skippableAfterTime = -1) 
	{
		...
		if ( skippableAfterTime >= 0 )
			yield return PowerQuest.Get.WaitForTime( skippableAfterTime );
		yield return PowerQuest.Get.WaitWhile( () => video.isPlaying, skippableAfterTime >= 0); // Wait until video stops playing
		video.Stop();
		if ( wasEnabled == false )
			video.enabled = false;
		yield break;
