...
{	
	...
	#if UNITY_EDITOR_WIN
	private const string defaultFont = "Consolas"; //"Segoe UI";
	#else
	private const string defaultFont = "Lucida Grande";		
	#endif
	...
	// Strings that should ahve the "yield return" removed from the start. The first group should contain whitespace
	...
	static readonly Regex[] REGEX_LOAD_MATCH = 
		{
			...
			new Regex(@"(^|\W)GlobalScript\.Script\.", RegexOptions.Compiled), 	// GlobalScript.Script. -> Globals. (Hack fix for hotloading. Needs GlobalScript class ref in hotloaded assembly file)
	...
	static readonly Regex[] REGEX_SAVE_MATCH = 
		{
			...
			new Regex(@"^(\s*)\.\.\.", RegexOptions.Compiled),
			new Regex(@"^(\s*)\.\.", RegexOptions.Compiled),
			new Regex(@"^(\s*)End", RegexOptions.Compiled | RegexOptions.IgnoreCase),
			new Regex(@"^(\s*)Consume", RegexOptions.Compiled | RegexOptions.IgnoreCase),
			...
			new Regex(@"(^|\W)Globals\.", RegexOptions.Compiled),
	...
	static readonly string[] REGEX_SAVE_REPLACE = 
		{
			...
			@"$1GlobalScript.Script.", // Globals.Script. -> GlobalScript.Script.
	...
	static readonly string STR_REPLACE_COLOR = @"$1<color=#009695>$2</color>"; // cyan : 009695 blue: 3364A4 pink: C12DAC
	static readonly string STR_REPLACE_COLOR_DIALOG = @"$1<color=#3364A4>$2</color><color=#F57D00>$3</color>";
	static readonly string STR_REPLACE_COLOR_COMMENT = @"<color=#888888>$1</color>";
	...
	static readonly Regex[] REGEX_COLOR = 
		{
			new Regex(@"^(\s*)(End)", RegexOptions.Compiled | RegexOptions.Multiline | RegexOptions.IgnoreCase),
			new Regex(@"^(\s*)(Consume)", RegexOptions.Compiled | RegexOptions.Multiline | RegexOptions.IgnoreCase),
			new Regex(@"^(\s*)(Return)", RegexOptions.Compiled | RegexOptions.Multiline | RegexOptions.IgnoreCase),
	...
	static void Open(GameObject gameObject, IQuestScriptable scriptable, eType type, string functionName = null, string parameters = "", bool isCoroutine = true )
	{
		...
		{
			// Open in IDE
			ViewInEditor(path, functionName);
		}
		else 
		{
	...
	void OnGuiNoFunction()
	{
		...
		currFunction = EditorGUILayout.Popup(currFunction,m_functionNames.ToArray(), new GUIStyle(EditorStyles.toolbarPopup) );
	...
	{		
		...
		{	
			...
			if ( (ev.type == EventType.KeyDown) && FindTextEditor() != null )
			{
				...
				else if ( ev.control == false && ev.keyCode == KeyCode.None && HasAutoComplete() && (ev.character == '\n' || ev.character == '.' || ev.character == ':' || ev.character == '\t') )
				{
					// Do Autocomplete
				...
				else if ( ev.control == false && ev.keyCode == KeyCode.None && ev.character == '\n' )
				{
					// Do carriage return
					forceIncrementUndoGroup = true;
		...
		// View Source
		//GUIContent testContent = new GUIContent(" C#", Contents.EYE.image, "View .cs script in IDE");
		//if  ( GUILayout.Button( testContent, Styles.TOOLBAR_BUTTON, GUILayout.MaxWidth(42) ) )
		...
		/* Trying save/compile on same button
		EditorGUI.BeginDisabledGroup(needsSave == false && needsCompile == false);
		if ( needsSave || needsCompile == false )
			doSave |= GUILayout.Button("Apply",  Styles.TOOLBAR_BUTTON, GUILayout.MaxWidth(80) );
		else if ( needsCompile )
			doAssetRefresh = GUILayout.Button("Compile", Styles.TOOLBAR_BUTTON, GUILayout.MaxWidth(80));		
		EditorGUI.EndDisabledGroup();
		*/
		...
		*/
		...
		// Sidebar
		EditorGUI.DrawRect(rect,UntintedColor(0.93f,0.93f,0.95f,1));

		// Border
		rect.xMin = rect.xMin+25; // TODO: need to add somethign to pad out the scroll view horzontally to account for this.
		EditorGUI.DrawRect(rect,UntintedColor(0.616f, 0.635f, 0.651f,1));

		// Text Area rect
		rect.width = rect.width-2;
		rect.x = rect.x+1;

		EditorGUI.DrawRect(rect,UntintedColor(1,1,0.95f,1));

		GUI.backgroundColor = UntintedColor(1,1,1,0);
		GUI.contentColor = UntintedColor(1,1,1,0);

		EditorGUI.BeginChangeCheck();

		Color cursorColor = GUI.skin.settings.cursorColor;
		Color selectionColor = GUI.skin.settings.selectionColor;
		GUI.skin.settings.cursorColor = UntintedColor(Color.black);
		...
		{				
			if (FindTextEditor().cursorIndex == FindTextEditor().selectIndex) 
		...
		if ( oldCursorIndex >= 0 )
		{
			FindTextEditor().cursorIndex = oldCursorIndex;
			FindTextEditor().selectIndex = oldCursorIndex;
		}
		...
		//
		{
			...
			GUI.color = UntintedColor(0.3f,0.3f,0.3f,1);
			GUI.contentColor = UntintedColor(0.3f,0.3f,0.3f,1);
	...
	void SetupTextStyle()
	{
		if ( s_textStyle == null || s_textStyle.font == null )
		{
			...
			s_textStyle.wordWrap = false;
			s_textStyle.font = Font.CreateDynamicFontFromOSFont(defaultFont, m_fontSize);
