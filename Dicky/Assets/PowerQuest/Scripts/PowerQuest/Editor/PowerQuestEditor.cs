...
public partial class PowerQuestEditor : EditorWindow 
{
	...
	public static readonly string SCRIPT_PARAMS_INTERACT_INVENTORY = " IInventory item ";
	...
	public static readonly string SCRIPT_PARAMS_USEINV_CHARACTER = " IInventory thisItem ";
	...
	{
		Undo.RecordObject(m_powerQuest, "Remove "+name);
		if ( EditorUtility.DisplayDialog("Really Remove?", "Dude, Sure you wanna remove "+name+"?", "Yeah Man", "Hmm, Nah") == false )
			return;
	...
	public static PowerQuestEditor Get { get{ return PowerQuestEditor.m_instance; } }
	...
	bool HotloadScripts()
	{
		...
		{
		    IQuestScriptable scriptable = scriptables[i];
			string[] assets = AssetDatabase.FindAssets(scriptable.GetScriptClassName()+STR_SCRIPT_TYPE,STR_SCRIPT_FOLDERS);
			if ( assets.Length == 0 )
				continue;
			
			string path = AssetDatabase.GUIDToAssetPath(assets[0]);//Path.GetDirectoryName( AssetDatabase.GetAssetPath(scriptable.GetPrefab()) ) + "/" + filename;
		...
		// Find all the paths we want to hot-load (parallel to hotloadedScriptables)
		...
		foreach( IQuestScriptable scriptable in hotloadedScriptables )
		{
			...
			if ( assets.Length > 0 )
			{
				string hotloadPath = AssetDatabase.GUIDToAssetPath(assets[0]);
	...
	public void PostProcessAnimationLists()
	{
		...
		catch
		{
			...
			Debug.Log("PostProcess exception");
		}
	}
