...
public partial class PowerQuestEditor
{
	...
	{	    
		...
		GUILayout.Label("Room: "+ m_selectedRoom.GetData().ScriptName + ( isPrefab ? " (Prefab)" : "" ), EditorStyles.boldLabel);

		GUILayout.Space(5);
		m_scrollPosition = EditorGUILayout.BeginScrollView(m_scrollPosition);

		GUILayout.BeginHorizontal();
		//GUILayout.Label("System Quest", EditorStyles.boldLabel); 
	...
	void OnSceneRoom(SceneView sceneView)
	{
		...
		if ( m_walkableAreaEditor != null && m_walkableAreaEditor.target != null )
		{
			// if ( m_listWalkableAreas != null && m_selectedRoom.GetWalkableAreas().IsIndexValid(m_listWalkableAreas.index)) // could move this so visible whenevern walkable areas are selected
			{
				...
				Handles.DrawAAPolyLine(4f,System.Array.ConvertAll<Vector2,Vector3>(walkablePoints, item=>item));	
				if (  walkablePoints.Length > 2 )
					Handles.DrawAAPolyLine(4f,walkablePoints[walkablePoints.Length-1], walkablePoints[0]);	
			}
