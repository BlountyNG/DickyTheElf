...
{	
	...
	static readonly string[] AC_KEYWORDS_C = { "Player","Plr","Ego" }; // trying variations on "player"
	static readonly string[] AC_KEYWORDS_I = { "Active", "Current" };
	static readonly string[] AC_KEYWORDS_D = { "Current", "option" };
	...
	static readonly Regex[] AC_CONTEXT_REGEX = 
	{
		...
		new Regex( @"^C\.\w+\.(\w*)$", RegexOptions.Compiled | RegexOptions.IgnoreCase ), 		// eg: C.Player.??
	...
	public void BuildAutoCompleteLists(bool onRoomChange = false)
	{
		...
		if ( m_scriptType == eType.Room || m_scriptType == eType.Hotspot || m_scriptType == eType.Prop || m_scriptType == eType.Region )
		{
			...
			if ( room == null )
				Debug.LogWarning("Autocomplete requires room reload");
		}
		else 
	...
	void UpdateAutoComplete(bool force = false)
	{
		...
		int expressionStart = m_text.LastIndexOfAny(AC_STARTSEQCHARS,Mathf.Max(0,index-1));
		...
		string expression = m_text.Substring(expressionStart, index-expressionStart);
		...
		for ( ; m_acContext < eAutoCompleteContext.Count; ++m_acContext )
		{
			if ( TryParse(m_acContext, ref expression) )
				break;
		}
		bool foundContext = m_acContext < eAutoCompleteContext.Count && m_acLists[(int)m_acContext] != null;
	...
	bool DoAutoComplete()
	{
		...
		string replaceText = m_acList[ Mathf.Clamp(m_acSelectedIndex, 0, m_acList.Count) ];
