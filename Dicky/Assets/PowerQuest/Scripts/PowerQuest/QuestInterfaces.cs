...
public partial interface IPowerQuest
{
	...
	/// Waits until the current dialog has finished
...
public partial interface IDialogOption
{
	...
	/// Whether the option is visible  (ie: on or shown)
	bool Visible { get; }
	/// Whether the option is disabled (ie: OffForever )
	bool Disabled { get; }
	/// Whether the option is marked as "seen"
	bool Used { get; set; }
...
public partial interface ICamera
{
	...
	/// Returns the index of teh character that the camera is following
	ICharacter GetCharacterToFollow();
	/// Sets which character the camera will follow
	...
	void Shake(float intensity, float duration = 0.0f, float falloff = 0.15f);
