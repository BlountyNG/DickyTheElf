...
{	
	...
	bool m_initialised = false; 	    // Flag set true after awake has finished and all questobjects have been created
	bool m_walkClickDown = false;       // Flag set when clicked to walk and reset when mouse button released
	bool m_guiConsumedClick = false;	 // Flags "game" mouse clicks to be ignored, resets after every update
	...
	// Hot-loading stuff
	Assembly m_hotLoadAssembly = null;
	...
	public bool ProcessClick( eQuestVerb verb, IQuestClickable clickable, Vector2 mousePosition )
	{
		...
		if ( clickedObj != null && clickHandled == false )
		{
			...
			// Clicked something! Work out what it was
	...
	void Update() 
	{
		...
		// Update timers
		for (int i = SV.m_timers.Count - 1; i >= 0 ; i--)
		{
		...
		//m_rightClick = Input.GetMouseButtonDown(1);
		...
		if ( m_overrideMouseOverClickable == false )
		{
			...
			// Check if mouse is over gui first
			if ( m_cameraGui != null )
			{			
				Vector2 guiMousePos = Input.mousePosition.WithZ(0);//m_cameraGui.WorldToScreenPoint( Input.mousePosition.WithZ(0) );
				m_mouseOverClickable = GetObjectAt(guiMousePos, (1<<LAYER_UI) ); // Find object under mouse in gui layer
				if ( m_mouseOverClickable != null )
					Debug.Log("Pointer over gui collision");
			}

			// First check if mouse is over gui
			...
			{			
				...
				{
					GuiComponent component = raycastResult.gameObject.GetComponentInParent<GuiComponent>();
					if ( component.GetData().Clickable )
					{
						m_mouseOverClickable = component.GetData();
						break;
					}
				}
			...
			// If mouse isn't over gui then check other clickables
	...
	bool StartScriptInteraction( IQuestScriptable scriptable, string methodName, object[] parameters = null, bool stopPlayerMoving = false )
	{	
		...
		try
		{
			result = StartScriptInteractionCoroutine(scriptClass, methodName, parameters, stopPlayerMoving);
			if ( result != null && result != m_consumedInteraction)
			{
				if ( m_currentSequence == null )
				{
					...
					// If first yield is walk, enable cancel
					if ( m_player.Walking && stopPlayerMoving && m_currentDialog == null )
					{
						EnableCancelInternal();
					}
				}
	...
	Coroutine StartScriptInteractionCoroutine( QuestScript scriptClass, string methodName, object[] parameters = null, bool stopPlayerMoving = false )
	{
		Coroutine result = null;
		if ( scriptClass != null )
		{
	...
	{		
		...
		bool first = true; // first frame the mouse will always be down, so don't skip until 2nd
		while ( ShouldContinueDialog(first, time, skippable, autoAdvance, audioSource) )
		{
			first = false;
			yield return new WaitForEndOfFrame();
			if ( SystemTime.Paused == false )
