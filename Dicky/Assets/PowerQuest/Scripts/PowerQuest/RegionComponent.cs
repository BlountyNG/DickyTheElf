...
public class RegionComponent : MonoBehaviour 
{
	...
	//List<Character> m_charactersInTrigger = new List<Character>();
	...
	{
		return m_characterOnRegionMask.Get( PowerQuest.Get.GetCharacterId(character) );
	}
	...
	public bool UpdateCharactersOnRegion( int index, Vector2 characterPos )
	{
		...
		m_characterOnRegionMask.Set(index,inside);
	...
	public eTriggerResult UpdateCharacterOnRegionState( int index )
	{
		...
		bool inside = m_characterOnRegionMask.Get(index);
		bool wasInside = m_characterOnRegionMaskOld.Get(index);
		...
		// update cached mask
		m_characterOnRegionMaskOld.Set(index, m_characterOnRegionMask.Get(index));
		return result;
	}
	...
	public void OnRoomLoaded()
	{
		...
		m_characterOnRegionMaskOld.SetAll(true);
		m_characterOnRegionMaskOld.And( m_characterOnRegionMask );
	}
	...
	void Start() 
	{
		if ( m_polygonCollider == null )
			m_polygonCollider = GetComponent<PolygonCollider2D>();
		m_characterOnRegionMask.Length = PowerQuest.Get.GetCharacterPrefabs().Count; // kind of hacky way to get this info :/
		m_characterOnRegionMaskOld.Length = m_characterOnRegionMask.Length;
