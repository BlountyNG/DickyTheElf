...
public class CharacterComponent : MonoBehaviour 
{
	...
	string m_animLastUpdate = null;
	bool m_flippedLastUpdate = false;
		...
		switch (m_state) 
		{
			...
			case Character.eState.Walk:
			{
				...
				{			
					...
					float speed =  (Mathf.Abs(direction.x) * m_data.WalkSpeed.x) + (Mathf.Abs(direction.y) * m_data.WalkSpeed.y);
			...
			case Character.eState.Animate:
			{
				...
				if ( m_spriteAnimator.Playing == false ) //m_spriteAnimator.GetCurrentAnimation() != null && m_spriteAnimator.IsPlaying() == false )
					SetState(Character.eState.Idle);
	...
	void LateUpdate()
	{	
		m_animLastUpdate = m_spriteAnimator.ClipName;
	...
	public void OnAnimationChanged()
	{
		...
		switch (m_state) 
		{
			...
			{
				// Play idle anim
				PlayAnimInternal(m_data.AnimIdle, false);
			} break;
			case Character.eState.Walk:
			{
				// Play walk anim
			...
			{
				// Play talk anim
				PlayAnimInternal(m_data.AnimTalk, false);
				if ( m_mouth != null )
				{
					bool flip;
	...
	{	
		...
		// Experimental "Transition" code. Not really working for flipping and stuff
		if ( string.IsNullOrEmpty(m_transitionAnim) == false && m_transitionAnim.StartsWith(animName, System.StringComparison.OrdinalIgnoreCase) == false )
		{
		...
		if ( clip != null )
		{
			string transitionName = GetTransitionAnim(m_animLastUpdate, m_flippedLastUpdate, clip.name, flip);
			if ( string.IsNullOrEmpty(transitionName) == false )
			{
				//Debug.LogFormat("{0} to {1} ({2}): found", m_animLastUpdate, clip.name, flip != m_flippedLastUpdate);
				// Found transition
			...
			{
				//Debug.LogFormat("{0} to {1} ({2}): NOT found", m_animLastUpdate, clip.name, flip != m_flippedLastUpdate);
			}
		}
		...
		if ( clip == null && Debug.isDebugBuild && animName != "Idle" && animName != "Talk"  && animName != "Walk" )
	...
	AnimationClip FindDirectionalAnimation( string name, out bool flip )
	{
	    ...
	    if ( string.IsNullOrEmpty( finalName ) )
	        return null;
		return GetAnimations().Find(item=>string.Equals(finalName, item.name, System.StringComparison.OrdinalIgnoreCase));
	}
